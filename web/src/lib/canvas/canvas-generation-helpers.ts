...
<<<<<<< HEAD
            ...
            // Stored images need a fresh object URL every session. Images kept as a remote link have no
            // storage key and resolve back to themselves, so a node mixing both still rebuilds correctly.
            if (node.metadata?.storageKey || !content.startsWith("data:image/")) return { ...node, metadata: { ...node.metadata, content: await resolveImageUrl(node.metadata?.storageKey, content), images } };
